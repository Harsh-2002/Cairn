...
Tags are key-value pairs attached to objects and to buckets, used to classify data for lifecycle and policy and for the operator's own organisation. The tagging body must be well-formed XML with each tag carrying a key and a value, and a malformed body is rejected. The S3 quantitative limits, namely a bounded number of tags per resource, bounded key and value lengths, and the permitted character set, are not yet enforced on write; that validation is a known limitation rather than a current guarantee. Object tags are attached to a specific object version, and a put can carry an initial tag set inline while the dedicated tagging operations get, replace, and delete a resource's tags afterward.
