...
- Embedded metadata store: SQLite. The recommended driver is the mature C SQLite compiled into the binary; a pure-Rust SQLite is tracked as a future swap behind the metadata abstraction (Section 11, Section 33). The operator's stated preference for a Rust-native SQLite is honoured by keeping the metadata backend behind a single interface so the engine can be replaced without touching protocol code.
...
- Operability: one binary that contains the server, the embedded UI, and the CLI subcommands. One coherent configuration surface from flags, environment, and an optional file.
...
A development authenticator that yields a fixed administrator principal exists solely to ease local development and is compiled only into development builds, and even then refuses to operate unless the listener is bound to a loopback address, so a production binary cannot bypass authentication (F-17). The first-start bootstrap, by which the very first administrator credentials are created into an empty user table, is exposed only on a loopback-only endpoint and only while no users exist, and returns the generated secret exactly once, after which only the hash and the encrypted SigV4 secret remain.
...
Cairn implements the S3 authorization concepts that govern access to buckets and objects: resource-based bucket policies, ACLs, Block Public Access, and Object Ownership. It has its own user and credential model rather than full AWS accounts and IAM, so where AWS would consult an identity-based IAM policy attached to a principal, Cairn substitutes its simpler user role model: an administrator is implicitly permitted, and a member's access to a resource is governed by the resource's policy and ACL. This is a deliberate approximation that keeps Cairn a storage server rather than an identity provider, and the boundary is stated so that conformance expectations are realistic: bucket-policy and ACL and public-access semantics are implemented faithfully, while cross-account principals, IAM roles, temporary credentials, and federation are out of scope (Section 2.3). Principals in a bucket policy are expressed either as the wildcard meaning anyone including anonymous, or as a specific Cairn user identity.
...
The API exposes an overview of the store, returning the bucket and object counts, the logical and physical storage totals and thus the achieved compression ratio, and summary replication health. It exposes listing and creating buckets, and a per-bucket detail that returns the bucket's size in logical and physical bytes, its object and version counts, its versioning and ownership state, and each of its configuration aspects, namely its policy, ACL, CORS, lifecycle, replication, tag set, and public-access-block settings, with operations to read and update each aspect so the UI can present and edit them; the same configuration is settable through the S3 subresource operations, and the management API is a convenience over the same stored state. It exposes a force-delete of a bucket that empties it by paging through its contents rather than loading it, for the common operator need to remove a populated bucket. It exposes a paged object listing for the data browser, including the ability to look at versions, and the minting of both S3 presigned URLs and Cairn signed public-read URLs for sharing or testing. It exposes user management, namely creating, listing, updating, and deactivating users and rotating their credentials. It exposes the activity and audit log with paging and time filtering. It exposes replication operations, namely listing failed replication entries and retrying them and viewing per-bucket replication status. And it exposes the non-secret portions of the running configuration and the health and readiness state. Every mutating endpoint records an audit entry.

### 22.3 Authentication and authorization of the control plane

The management API authenticates with the same credential mechanisms as the rest of the system, with the Bearer scheme being the natural fit for the UI and the CLI, and it requires the administrator role for all operations, refusing members and anonymous requests. The UI obtains a session by authenticating once and then carries it on subsequent calls, with the session handling designed to resist cross-site request forgery given that the UI is served from the same origin as the API. Because the control plane can read and change everything, it is held to the same wire-security expectations as the rest of the system: it is served over TLS, whether terminated by Cairn or by a proxy, and credentials never traverse an untrusted hop in clear.
...
The server serves the embedded single-page application under a management UI path, returning the application shell for client-side routes so that the framework's routing works on reload, and serving the script and style assets with appropriate caching headers since they are content-hashed by the build. The application is a client of the management API: it authenticates the operator, then renders the store overview with the storage and compression and replication figures, a bucket view for listing and creating buckets and for inspecting and editing each bucket's versioning, policy, ACL, CORS, lifecycle, replication, tagging, public-access-block, and compression settings, a data browser for paging through objects and versions and for uploading, downloading, deleting, and generating share URLs, a user-management view, an activity and audit view, and a replication-status view that surfaces lag and failures and offers retry. The UI changes nothing that the API does not expose, so it carries no privileged logic of its own and remains a thin presentation over the control plane.

### 23.4 Security posture of the UI

Because the UI is served by the same process and talks to the same admin-gated API, its security reduces to the API's: it requires an administrator session, it is served over TLS, and it is subject to the same audit logging for the actions it triggers. Serving the UI from the same origin as the API simplifies the session and cross-site-request-forgery handling, and the UI exposes no secret material beyond what an administrator is entitled to see, with credentials shown once at creation and only hashes and ciphertext retained thereafter.
...
As an API client the CLI offers commands mirroring the management API: creating, listing, and removing buckets and force-emptying them; reading and writing each bucket configuration aspect, taking policy and CORS and lifecycle and replication documents from files so they can live in version control; managing users and rotating credentials; browsing and listing objects and versions; uploading, downloading, and removing objects; minting presigned and public-read URLs; viewing the store overview and metrics; and viewing and retrying replication. It is configured by flags, environment, and an optional configuration file giving the endpoint and credentials, and it can emit either human-readable output for interactive use or structured output for scripting, so it composes into automation.
...
| Setting an ACL while ownership disables ACLs | 400 | InvalidRequest (the ownership-specific message) |
...
Configuration comes from command-line flags, environment variables, and an optional configuration file, layered so that a flag overrides an environment variable, which overrides a file value, which overrides the built-in default, and it is validated on load so that an invalid configuration fails fast with a clear message rather than at first use. Every setting is available through both an environment variable and a flag following the project's naming convention; the table below names settings logically. The settings continue the operational vocabulary an operator coming from a comparable server would expect.

### 28.2 Settings

| Setting | Default | Meaning |
|---|---|---|
| Listen address | loopback on the service port | Where the server binds. |
| TLS certificate and key paths | unset (plaintext) | Enable built-in TLS when set; otherwise serve plaintext behind a proxy. |
| Database path | under the data root | Location of the SQLite metadata file. |
| Data directory | a data root | Root of the staging and per-bucket blob directories; must share a filesystem with the database. |
| Public base URL | unset | External base URL used when generating URLs behind ingress. |
| Metadata cache budget | a few hundred mebibytes | Byte budget for the metadata and configuration cache; zero disables it. |
| SQLite synchronous level | full | Full for durability against power loss, or a relaxed setting for throughput at the risk of losing the last few commits on power loss. |
| Blob fsync policy | always | Fsync each blob and its directory on commit, or relax for throughput on non-critical data. |
| Read connection pool size | about the core count | Number of read-only metadata connections. |
| Blob I/O pool size | tuned to the device | Threads dedicated to blob file I/O. |
| Group-commit linger | off or a few hundred microseconds | Optional wait to enlarge write batches under bursty load. |
| Maximum object size | a large ceiling | Hard per-object size limit. |
| Per-bucket and per-user quotas | unset | Optional byte quotas enforced at commit. |
| Write-ahead-log checkpoint interval | on the order of a minute | Cadence of the truncating checkpoint. |
| Reconcile on start | true | Run the full reconciliation before serving, or defer it on very large stores. |
| Multipart session lifetime and sweep interval | a day and an hour | When idle uploads become stale and how often the sweeper runs. |
| Lifecycle scan interval | on the order of hours | How often the lifecycle scanner runs. |
| Replication worker count and backoff bounds | a small pool and capped exponential backoff | Replication concurrency and retry behaviour. |
| Compression defaults | disabled | The default per-bucket compression policy when a bucket does not specify one; algorithm and block size when enabled. |
| Master key | required when any encrypted secret exists | The authenticated-encryption key for secrets at rest, supplied out of band. |
| Concurrency limit and request timeouts | bounded defaults | Maximum in-flight requests and per-phase timeouts. |
| Public-read signing secret and default and maximum URL lifetimes | unset until configured | The keyed-hash secret and lifetimes for Cairn's signed public-read URLs. |
| CORS policy for the management API | a configured set | The fixed cross-origin policy for the management surface, distinct from per-bucket S3 CORS. |
| Log level and format | informational and text | Verbosity and whether logs are text or JSON. |
| Metrics exposure | on the service listener | Where the metrics endpoint is served. |

Validation rejects an empty listen address or paths, a public base URL that does not parse, TLS configuration that is incomplete, a public-read secret that is too short, a default URL lifetime exceeding the maximum, non-positive timeouts, a sweep or scan interval below a sane floor, a development bind that is not loopback, and the presence of encrypted secrets without a master key.
...
The decisive tests run real S3 clients and the de-facto conformance suite against a running Cairn. The AWS Rust SDK drives a matrix covering the object operations including plain, unsigned-payload, and streaming-chunked puts so that the chunked path is exercised by a real client, ranged and conditional gets, heads, deletes, bulk deletes, copies, the full multipart cycle including abort and out-of-order completion, and presigned URLs, together with the new surface, namely versioning behaviour and version listing, tagging, ACL and policy and public-access-block effects, CORS preflight and actual-request handling, and lifecycle and replication configuration round-trips. The same core operations are smoke-tested through the MinIO client, through rclone, and through the older command-line client, because clients differ in signing defaults, header casing, and listing expectations, and passing all of them is the practical definition of compatibility. The widely-used community S3 conformance test suite, which exercises a broad range of bucket, object, ACL, policy, versioning, tagging, CORS, and multipart behaviours against a server, is run as a gate, and the subset Cairn intends to support is expected to pass, with the unsupported areas explicitly marked. Replication is tested end to end between two Cairn instances and with a fake sink that can simulate failures to exercise retry and backoff; lifecycle is tested with a controllable clock so that expiry, transition, and abort timing are deterministic; and compression is tested for round-trip fidelity, for correct ranged reads against compressed blobs, for the incompressibility heuristic, and for ETag invariance between compressed and uncompressed storage of the same content.
...
The single writer is a real ceiling on small-object write throughput, the same ceiling the reference design has, now made explicit, raised by group commit, and instrumented. The write-queue-depth metric is the operator's window onto it: a depth that grows under load means small-object writes are the binding constraint, at which point the operator can enlarge the group-commit linger to trade a little latency for more batching, relax the synchronous or blob-fsync setting if the durability trade is acceptable for the workload, or accept the ceiling as the honest limit of a single node and scale by running more nodes with replication. Stating this plainly is part of being a credible production system.

### 30.3 Tuning knobs and their effects

The synchronous level and the blob-fsync policy trade durability for small-write rate. The group-commit linger trades latency for batching. The read-pool size raises read concurrency up to the core count, beyond which it adds contention without benefit. The metadata cache budget keeps hot metadata and configuration resident, making hits cheap reference-count operations. The blob I/O pool size matches the device's useful concurrency. The compression algorithm and level, and the choice to bypass the page cache for large writes and to advise the kernel about access patterns, tune the byte path for the workload. Memory-mapping and cache sizing on the database reduce read syscalls. Each knob has a defined effect, so tuning is reasoning rather than guessing.
...
**Phase 15, future work behind the interfaces.** The io_uring blob engine and the zero-copy read fast path with kernel TLS, an explicit restore-from-cold workflow, server-side encryption at rest as a blob-store decorator, a swap of the metadata engine to a pure-Rust SQLite, and the protective version-deletion control. None of these require changes to the protocol or control layers, which is the payoff of the abstraction boundary.
...
**Bucket configuration aspects.** Each of the following is a validated document associated with a bucket, stored either as a column on a bucket-configuration table keyed by bucket name or as its own table; the choice is an implementation detail, but each is one logical document per bucket: the policy, the access-control list, the CORS configuration, the lifecycle configuration, the replication configuration, and the tag set. The account-wide and per-bucket public-access-block settings are stored as their four boolean toggles.
...
| compression | text | Not null; the algorithm and block size, or a marker that the blob is uncompressed; for transitioned objects, the cold-tier locator. |
...
| Server-side encryption operations, object lock and retention, requester pays, website and accelerate and analytics and inventory and metrics configurations | No | Out of scope; requests are answered as not implemented. |
