...
async fn list_impl(
    ...
    'outer: loop {
        ...
        for summary in rows.into_iter().take(LIST_BATCH) {
            ...
            if let Some(delim) = query.delimiter.as_deref().filter(|d| !d.is_empty()) {
...
impl MetadataStore for AsyncMetadataStore {
    ...
    async fn is_bucket_empty(&self, name: &BucketName) -> Result<bool, MetaError> {
        let row = query_one(
            &**self.reader().await,
            // Empty means NO object_versions rows AND no in-progress multipart uploads, matching
            // S3 DeleteBucket semantics (audit #3; multipart added 2026-07).
    ...
    async fn get_import_job_record(&self, id: &str) -> Result<Option<ImportJobRecord>, MetaError> {
        let row = query_one(
            &**self.reader().await,
            &format!("SELECT {IMPORT_JOB_RECORD_COLS} FROM import_jobs WHERE id=?1"),
            vec![Value::Text(id.to_owned())],
        )
        .await?;
        row.map(|r| model::import_job_record_from_row(&r))
            .transpose()
    }
