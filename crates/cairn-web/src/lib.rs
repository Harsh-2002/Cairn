//! `cairn-web` — the embedded React management single-page application (ARCH 23).
//!
//! The management web console is a React SPA built by Vite into `web/dist`. That bundle is
...
//! with no separate web console service to deploy or version-match (ARCH 23.1).
//!
//! This crate exposes a tiny serving surface that the server's management-web console route
...
/// The built static asset bundle (`web/dist`), embedded at compile time.
///
...
/// built (`npm run build` in `web/`) before this crate compiles.
...
///
/// ```
/// let (content_type, body) = cairn_web::asset("index.html").expect("index embedded");
...
///
/// The server returns this for management-web console paths that are not concrete assets so
...
pub fn spa_shell() -> (String, Cow<'static, [u8]>) {
    asset(INDEX).expect("web/dist/index.html must be embedded; build the front-end first")
}
...
mod tests {
    ...
    /// `web/dist` (e.g. a forgotten `npm run build`): if Vite emitted an index
        ...
        for needle in ["src=\"", "href=\""] {
            ...
            while let Some(start) = rest.find(needle) {
                ...
                let (content_type, bytes) = asset(raw).unwrap_or_else(|| {
                    panic!("index references {raw}, which is not embedded; rebuild web/dist")
                });
