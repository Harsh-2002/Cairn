...
Embeds the built React management SPA (`web/dist`) into the server binary via `rust_embed`, so a
Cairn deployment is one binary with no separate web console service to deploy or version-match (ARCH 23.1).
This crate is the *embed + serve* surface only — the React/TypeScript source lives in **`web/`** (a
separate npm project, excluded from the cargo workspace; see `../../web/CLAUDE.md`).

## Layout (`src/`)
- `lib.rs` — the whole public surface. `Assets` (`#[folder = "../../web/dist"]`) is the compile-time
...
- `build.rs` — scaffolds a placeholder `web/dist/index.html` only if one doesn't already exist, so
