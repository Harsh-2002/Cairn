...
pub fn reserve(
    ...
    x(
        db,
        "INSERT INTO storage_write_intents(attempt_id,generation,bucket_name,plan,created_at) VALUES (?1,?2,?3,?4,?5)",
        vec![
...
fn load(
    ...
    q(db, "SELECT plan,attempt_id,generation,bucket_name FROM storage_write_intents WHERE attempt_id=?1 AND generation=?2 AND bucket_name=?3",
        vec![text(attempt.as_str()),text(generation.as_str()),text(bucket.as_str())])?
...
fn resolve(db: &DB, plan: &StorageWritePlan) -> R<()> {
    for path in &plan.paths {
        if !referenced(db, &path.path)? {
            enqueue(db, &plan.bucket, &path.path, None)?;
        }
    }
    x(
        db,
...
pub fn enqueue(db: &DB, bucket: &BucketName, path: &StoragePath, quota: Option<&str>) -> R<()> {
    ...
    x(
        db,
        "INSERT INTO storage_cleanups(id,storage_path,bucket_name,quota_debt_id) VALUES (?1,?2,?3,?4)",
        vec![
