...
pub fn claim(db: &DB, limit: u32, now: Timestamp, lease_secs: i64) -> R<MutationOutcome> {
    let until = lease(now, lease_secs)?;
...
fn decode(row: &rusqlite::Row<'_>) -> rusqlite::Result<RemoteMultipartUpload> {
    ...
    Ok(RemoteMultipartUpload {
        id: row.get(0)?,
        outbox_id: row.get(1)?,
        origin_token: ReplicationClaimToken::from_string(row.get(2)?),
        destination,
        upload_id: row.get(4)?,
        cleanup_token: row
            .get::<_, Option<String>>(5)?
            .map(ReplicationClaimToken::from_string),
        lease_until: row.get::<_, Option<i64>>(6)?.map(Timestamp),
        next_attempt_at: Timestamp(row.get(7)?),
        orphan_reported: row.get(8)?,
...
fn origin_owned(
    ...
    db.query_row("SELECT 1 FROM replication_outbox WHERE id=?1 AND bucket_name=?2 AND status='claimed' AND claim_token=?3 AND lease_until>=?4",params![id,bucket.as_str(),token.as_str(),now.0], |_| Ok(())).optional().map(|row| row.is_some()).map_err(engine_err)
}
fn insert(db: &DB, row: &RemoteMultipartUpload) -> R<()> {
    let destination = serde_json::to_string(&row.destination)
        .map_err(|error| MetaError::Engine(error.to_string()))?;
    db.execute("INSERT INTO replication_uploads(id,bucket_name,outbox_id,origin_token,destination,next_attempt_at) VALUES (?1,?2,?3,?4,?5,?6)", params![row.id,row.destination.bucket.as_str(),row.outbox_id,row.origin_token.as_str(),destination,row.next_attempt_at.0]).map_err(engine_err)?;
    Ok(())
}
fn save(db: &DB, row: &RemoteMultipartUpload) -> R<()> {
    db.execute("UPDATE replication_uploads SET upload_id=?2,cleanup_token=?3,lease_until=?4,next_attempt_at=?5,orphan_reported=?6,last_error=?7 WHERE id=?1",params![row.id,row.upload_id,row.cleanup_token.as_ref().map(ReplicationClaimToken::as_str),row.lease_until.map(|time|time.0),row.next_attempt_at.0,row.orphan_reported,row.last_error]).map_err(engine_err)?;
    Ok(())
}
fn remove(db: &DB, id: &str) -> R<()> {
    db.execute("DELETE FROM replication_uploads WHERE id=?1", params![id])
        .map_err(engine_err)?;
    Ok(())
}
fn due(db: &DB, limit: u32, now: Timestamp) -> R<Vec<RemoteMultipartUpload>> {
    let mut statement=db.prepare(&format!("SELECT {COLS} FROM replication_uploads AS u WHERE orphan_reported=0 AND next_attempt_at<=?1 AND (cleanup_token IS NULL OR lease_until<?1) AND NOT EXISTS (SELECT 1 FROM replication_outbox AS o WHERE o.id=u.outbox_id AND o.bucket_name=u.bucket_name AND o.status='claimed' AND o.claim_token=u.origin_token AND o.lease_until>=?1) ORDER BY next_attempt_at,id LIMIT ?2")).map_err(engine_err)?;
    statement
        .query_map(params![now.0, limit], decode)
        .map_err(engine_err)?
