...
# the web-UI listener on and prove the httpOnly session-cookie auth flow end to end with plain curl
...
#     plane on the UI port (CreateBucket / PutObject / GetObject round-trip).
...
st="$(code -c "$JAR" -X POST -H 'Content-Type: application/json' \
  -d "{\"access_key\":\"$AK\",\"secret_key\":\"$SK\"}" "$UI/api/v1/session")"
...
st="$(code -X POST -H 'Content-Type: application/json' \
  -d "{\"access_key\":\"$AK\",\"secret_key\":\"definitely-wrong\"}" "$UI/api/v1/session")"
...
st="$(code -b "$JAR" -c "$JAR" -X DELETE "$UI/api/v1/session")"
