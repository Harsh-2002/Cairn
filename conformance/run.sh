#!/usr/bin/env bash
# Run the boto3 conformance suite against a freshly-bootstrapped Cairn server.
# Usage: BIN=target/debug/cairn PY=python3 conformance/run.sh
set -euo pipefail
...
"$PY" "$(dirname "$0")/conformance.py" "$AKID" "$SECRET" "http://127.0.0.1:$PORT"
