...
| 2026-09-08 | Approved storage lifecycle protocol 2 | Implementation integrated locally; full gate and merge pending | v36/v37 journal and multipart alias ownership now cover production creation, publication, deletion, recovery and quota across SQLite/libSQL/Turso/doubles/shards. Mandatory planned admission and actual backend/node lifetime replace direct request cleanup. Five real-filesystem startup tests pass; full scans preserve every intent/claimed debt path. The actual io_uring SIGKILL fixture proves exclusion through observed pending teardown, not power loss or an observed post-exit reference window. Full CI, admission-cost evidence and 3C merge remain pending; coverage remains incomplete and 3D is unactivated. |
| 2026-09-08 | Async Writer savepoint failure handling | Local fix tested; issue filing approval and integration pending | Merged main bf17dfe ignores failed savepoint release/rollback in the optional async Writer. A deterministic partial-write/failed-rollback fixture now causes whole-batch abort and preserves the original row. Typed capacity errors survive fan-out in SQLite/libSQL/Turso, while committed-then-error fixtures retain ambiguity. Found during PR #91 CI investigation; details are in docs/storage-review-2026-09.md. |
