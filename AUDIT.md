...
| 2026-09-07 | Replication intent resolution | Locally fixed and tested; PR pending | Read/parse errors fail affected writes before commit; bulk markers share resolution while permanent version deletes remain independent. Cleanup/retry regression covers injected read faults and malformed config on in-memory and SQLite backends. |
| 2026-09-06 | Release verification | Locally implemented; not merged | Installer requires signed checksums/binaries and digest-pinned signed containers with release/commit-bound provenance; pinned verifier bootstrap, no bypass. |
