...
| 2026-09-25 | Trusted-proxy startup-error CodeQL #45 | Backup-topology stderr sink fix pending exact-head CI | `Config::trusted_proxy_allowlist` previously included a rejected `CAIRN_TRUSTED_PROXIES` value in its `ConfigError`; `main` printed that error to stderr. PR #111 made the proxy error fixed and PR #112 made the generic `Config::load()` failure output constant, but fresh main CodeQL still traced the config value to `require_canonical_backup_topology`'s stderr output. That helper's error is now also constant, preserving the same single-SQLite refusal. Real-binary regressions check both startup and backup error output. Do not close this finding until fresh main CodeQL changes the alert state. |
