...
- `install.sh` — **rechecked 2026-09-25:** requires a checksum tool, verifies a checksum-pinned
  Cosign bootstrap, signed `SHA256SUMS`, the release binary's checksum and signature before replacing
  the installed binary; missing or failed verification aborts. It generates
  `CAIRN_MASTER_KEY`/`CAIRN_ROOT_SECRET_KEY` via `openssl rand -hex 32` and writes them to the
  restricted installation environment, but still **prints the access/secret key to stdout** on
  completion (see Section 4.1). Re-verify these properties against code on later runs.
...
- **Release/installer distinction, rechecked 2026-09-25:** the release workflow signs artifacts
  and publishes SBOM/provenance with separated job authority (`SECURITY.md`,
  `tests/release_policy.py`); the installer now enforces signature and checksum verification for
  release binaries and verifies the immutable container image and provenance. Do not refile the
  previously fixed consumption-verification gap.
...
|---|---|---|---|
| 2026-09-25 | Performance-instrumentation differential audit and evidence hygiene | No new production security or durability defect found; no optimization adopted | Reviewed the current dirty-tree Rust timing paths, bounded metrics, benchmark runners and retained reports against ARCH 8/26/29–30 and the audit playbook. Namespace parent barriers, staged-file sync, rename, destination sync and Writer publication remain ordered; sample rings and metric labels are bounded. Corrected stale installer-verification guidance and one misleading metric comment. Moved 26 referenced, hash-retained JSON reports into `docs/performance-evidence/2026-09/`; no evidence was discarded. The full pinned local gate passed 1,511/1,511 workspace tests (5 skipped; one slow), both Clippy configurations, doctests, formatting, web lint/build/browser checks/audits, installer regressions, Python runner tests and policy tests. Cargo audit found no vulnerability; updated yanked `chacha20` 0.10.0 to 0.10.2, leaving the previously allowed unmaintained `rustls-pemfile` warning. Shared-node benchmark rankings remain inconclusive and are not a release claim; branch CI, deployment and power-loss validation are not implied by this local audit. |
...
| 2026-09-09 | Bootstrap Bearer export CodeQL #44 | Reviewed; retained explicit CLI output (`won't fix`) | PR #103's shared credential encoder adds a tracked secret-to-stdout path at `crates/cairn-server/src/main.rs:2769`. This is the same node-local `bootstrap` export accepted for #31/#32 and documented in ARCH 14.5/24.3. The command intentionally prints operator credentials; captured stdout remains sensitive. The console/session and server logging paths do not print the encoded token. No logging-rule suppression or production vulnerability fix is claimed. |
| 2026-09-25 | Trusted-proxy startup-error CodeQL #45 | **Fixed on main** in PRs #111–#113 | `Config::trusted_proxy_allowlist` previously included a rejected `CAIRN_TRUSTED_PROXIES` value in its `ConfigError`; `main` printed that error to stderr. PR #111 made the proxy error fixed, PR #112 made the generic `Config::load()` failure output constant, and PR #113 made the backup-topology refusal constant while preserving the same single-SQLite guard. Real-binary regressions check startup and backup stderr. A separately dispatched full CI/CodeQL run on exact merged main `d0dcd5d` passed; GitHub changed alert #45 to `fixed` at 2026-09-25 10:47 UTC. No alert dismissal was used. |
| 2026-09-09 | Repeated CodeQL nonce-test alert #41 | Test-only fixture corrected | The constants date to `6d9f69f` and remain inside `#[cfg(test)]`. Alert #37 was dismissed, but the separate #41 record stayed open. The nonce regression now generates a key and flips one bit for a guaranteed distinct second key; determinism, block separation and key separation assertions remain. No production hard-coded key was found. |
...
| 2026-09-08 | PR #93 CodeQL review | No new issue; branch alerts #42/#43 remediated in source | The fixed test-only DEK was replaced by a generated key. The baseline topology refusal now emits a constant message instead of the helper's configuration-derived error; the helper only reports backend/shard topology, with no demonstrated secret disclosure. No alert dismissal or guard weakening. Complete Rust gate passed again (1,466 default / 1,492 all-feature tests); corrected-head CI pending. |

| 2026-09-08 | Isolated phase 4A packing publication review | None; no new blocking finding | Reviewed durable receipt/root binding, exact SQL span/condition checks, encrypted declaration persistence, shared descriptor pinning and cancellation through actual blocking work. 37 Rust tests and 12 coordinator tests pass, including real tiny files/packed/unknown fixtures. FULL actor is laboratory-only; no production format, S3 parity, collection/restore completion or performance adoption claim. Complete workspace gate passes (1,466 default / 1,492 all-feature tests); all 47 Rust lab and 70 Python lab tests pass with every live fixture enabled. Final-head CI pending. |

| 2026-09-08 | Phase 4A CI initialization follow-up | No production finding | Repeated Chrome initialization timeouts now have a fixed 30-second allowance, bounded stderr and six ownership/chunk/deadline fixtures; product performance assertions are unchanged. Initial mixed-feature soak WAL high-water exceeded 512 MiB; unchanged retry passed with the existing ceiling. Preserve the initial result in the packing record. All 54 checks passed at 825d543; PR #94 merged as 3b2edbd with no open branch alerts or review comments. |
| 2026-09-08 | Isolated phase 4B collection and offline recovery integration | None; laboratory review fixes included | Bounded candidate/copy admission, FULL exact-location relocation, durable physical charges, shared reader lifetime and typed offline snapshot/restore. Review corrected per-entry allocation reservation and rejected hard-linked SQLite database/sidecar aliases before SQLite opens. All 66 packing tests, 76 standalone Rust tests and 70 live-enabled Python tests pass; full gate passes 1,466 default / 1,492 all-feature tests. No production format/backup connection, performance selection or power-loss claim. Final-head CI required. |

| 2026-09-09 | Phase 4C whole-pipeline comparison review | No new production finding | Fixed transpose and scattered survivor checks exercise mixed live/dead collection, exact overwrite/delete, ranges, offline snapshot/fresh restore/reopen under shared admission. Predeclared ninety arms at C32/1,024 objects protect unknown and >threshold dedicated files without rejecting smaller thresholds based on unused large packed policies. All 70 packing / 80 Rust lab tests and 84 live-enabled Python tests pass; full workspace gate passes 1,466 default / 1,492 all-feature tests. No measurement or adoption decision yet. PR #95 passed all 54 checks and merged as ea553fc. |

| 2026-09-09 | Phase 4C bounded evidence and decision | No new production finding | Eleven arms passed required correctness/observations; the first 1-MiB packed arm reached its fixed deadline. The incomplete matrix is INCONCLUSIVE / KEEP files; no threshold or production change. All raw evidence is archived before owned process/data/artifact cleanup. The comparison/export charged 326.493302 seconds; campaign 918.639472 seconds, 1,778,102,272 peak bytes. Full local gates passed; final-head CI pending. |
| 2026-09-09 | Populated metadata laboratory and capability comparison | No production finding; hot-bucket Writer criterion qualifies | Both canonical FULL 100,000-row populations and all family/quota/reopen checks passed. Hot C128 occupancy/queue are 95.22%/81.87%, with 10.26% gain over C32 and stable controls. Distributed queue 79.77% misses the 80% predicate. Complete stage evidence is Writer service, not pure SQLite CPU. Measurement/export charged 292.003397 seconds; campaign 1,210.642869 seconds and 1,778,102,272 peak bytes. Evidence is archived and original owned data/processes removed. SQLite remains production; the standalone Fjall comparison is conditional only on the hot population. Full local root gate, 98 laboratory Rust tests (one ignored subprocess helper), and all 104 live Python fixtures pass; final-head CI pending. |
| 2026-09-09 | Conditional Fjall laboratory ownership and failure review | No production finding; fixed-trace adapter only | Pinned 3.1.10/SyncAll actor stages independent failed members before batch commit; exact indexes cover the populated 18-family workload. Tiny trace/128-owner/reopen fixtures and byte-for-byte late-outbox rollback pass. Native journal EFBIG returns no success; SIGKILL before commit and after durable commit/before reply preserves atomic state on reopen. Reads and close retain actual I/O owners. Observer errors drain owned work; final process I/O survives unavailable zombie `/proc` samples. Unsupported surfaces fail closed, and full production semantic/migration/backup/power-loss parity remains unproven. Paged terminal-outbox retention and missing-seed resume refusal have regression coverage. Full local gates pass. Five-pair criteria precede timing; no alternative capacity result yet. |

| 2026-09-09 | Phase 5B bounded evidence / Phase 5C decision | No production finding; INCONCLUSIVE / KEEP SQLite | SQLite's 100,000-row seed completed in 95.338541 seconds; Fjall reached its fixed 105-second preparation deadline. Zero load arms/pairs ran, so no replacement gain or protected-metric result is claimed. Process/data cleanup and hash-verified raw export passed. Metadata charged 496.775609 seconds cumulative; complete campaign 1,415.415082 seconds / 1,778,102,272 peak bytes. Local full gate and 98 Rust / 104 live Python lab fixtures pass; final-head CI/merge pending. No automatic retry or production dependency change. |

| 2026-09-09 | Final storage-evolution integration | Closed implementation and bounded decisions; no new finding | Phase 5 merged in PR #97 as `30d1f64` after all 54 checks passed for `bd9999e`; reviews/comments and open branch code-scanning alerts were empty. Earlier Phase 4C CI/integration is complete in PR #96. All five phases are delivered; production retains files, full startup scans and SQLite. The measured hot-bucket Writer limit is scoped; the alternative preparation deadline yields no replacement gain. All owned measurement data/processes/raw working artifacts and implementation worktrees/merged branches are removed. Checked evidence and the final budget remain tracked. |

| 2026-09-09 | CodeQL #41 and release integration root cause | Existing test-only alert; no new production vulnerability | The nonce constants date to `6d9f69f` (June 7). Generated key material preserves all three nonce assertions and the exact regression passes. Release hardening in `ffc9c8d` followed the last successful July 25 release: its renamed wheel, numbered-toolchain versus repository `stable` override, and custom SLSA build types were not exercised by the static policy gate. Wheel/toolchain regressions now cover the first two. GitHub-compatible workflow provenance retains exact Cairn claims; tests execute both producers and all eight consumers, rejecting altered identity, recipe, version, target, digest and attempt. Installer verification consumes the new schema. Full CI and actual publication remain mandatory release gates; no alert suppression, authority expansion or verification bypass was added. |

| 2026-09-09 | PR #103 CodeQL #44 credential-export review | Existing accepted CLI behavior; no new finding | Reviewed source 4709a52 and prior dispositions #31/#32. The shared encoder flows into the documented explicit node-local bootstrap credential output, with no new HTTP/server logging sink. Alert #44 was dismissed as `won't fix`, retaining the operator export and its stdout exposure. All PR checks passed, including CodeQL after disposition; merged as 789f23e. Local full gate, exact-string login/CLI checks and desktop/mobile autofill regressions passed. |
| 2026-09-25 | Full local gate and differential audit of performance evidence/instrumentation | No new issue filed; existing hardening items remain in the register | Reviewed changed Rust, Python and documentation against the storage ordering and bounded-observability contracts. Both warnings-denied Clippy legs, 1,511 workspace tests (5 skipped), doctests, formatting, web lint/build/browser performance checks and both npm audits, installer tests, benchmark-runner tests and release/CI policy tests pass. Cargo audit reports no vulnerability after replacing yanked `chacha20` 0.10.0 with 0.10.2; `rustls-pemfile` remains an allowed unmaintained warning. Installer verification notes were corrected. Twenty-six referenced raw JSON reports were organized under a dated evidence directory without deleting their hash-bound data. This is a local gate on an audit feature branch, not full manual proof of every code path, exact-head GitHub CI, a release, deployment or a new performance adoption. |
| 2026-09-25 | PR #110 first CI review and all-feature timing-test correction | No production security or durability finding; merge remains gated | The first all-features job exposed backend-specific assumptions in three new sampled-timing tests: enabling `io-uring` omits Tokio-only finalization substage samples. The cancellation fixture also signaled before its pending body read, allowing abort before the incomplete input-wait sample existed. The tests now explicitly select the Tokio backend and signal only when the pending read is polled; all 107 runnable `cairn-blob` all-feature unit tests pass locally (2 ignored). The same first run's mixed-feature soak completed 45,936 operations with zero errors and passing correctness/plateau checks, but exceeded the WAL peak backstop (694,079,488 versus 536,870,912 bytes). That failure is retained, not waived; exact-head CI must pass before merge. |
| 2026-09-25 | Main post-merge CodeQL/dependency/secret-alert follow-up | One existing CodeQL #45 source path fixed locally; no new public issue filed | GitHub reported one open CodeQL alert on `main` (#45, cleartext logging), zero open Dependabot alerts and zero open secret-scanning alerts. Traced #45 from a malformed administrator-supplied trusted-proxy value through config validation to startup stderr; replaced only the outward error with a fixed diagnostic and added a non-echo regression. The parser still rejects the same values. This does not certify the whole product or close the GitHub alert until a clean analysis runs on `main`; full gate and PR review are required before merge. |
| 2026-09-25 | Main CodeQL #45 re-analysis and WAL soak review | Alert remains open; second source fix in progress | PR #111 passed all 59 exact-head checks and merged as `cee0014`, but an independently dispatched full CI on `main` found the same cleartext-logging path at the generic `Config::load()` stderr sink. The proxy-specific error redaction was insufficient. A second fix replaces that sink with fixed output and exercises the actual binary with sensitive-looking invalid environment values. The same main run's soak again exceeded only its 512-MiB WAL peak backstop (712,401,592 bytes); its 32,687 operations had zero errors, byte-integrity and plateau checks passed. No ceiling was relaxed and neither failure is marked resolved by this audit entry. |
| 2026-09-25 | Recurrent WAL peak under mixed-feature soak | Fixed in PR #112; merged-main soak passed | The PR #112 initial strict soak again peaked at 709,756,552 bytes against the unchanged 536,870,912-byte ceiling. Its 33,073 operations had zero errors, byte-integrity and plateau checks passed. The 64-MiB size-triggered checkpoint was inspected only once per ten seconds, allowing hundreds of MiB of writes between probes. The merged fix polls the existing sidecar once per second when the size trigger is enabled, retains the same checkpoint conditions and writer-thread busy behavior, and leaves the soak gate unchanged. The cadence regression and separately dispatched exact-main mixed-feature soak passed at `6187907`; this is a CI backstop result, not a worst-case WAL size guarantee. |
| 2026-09-25 | PR #112 and exact merged-main re-analysis | WAL gate passed; CodeQL #45 still open | All 59 exact-head PR checks passed and PR #112 merged as `6187907`. A separately dispatched full CI on that exact main commit passed, including mixed-feature soak under the unchanged WAL ceiling and Rust CodeQL analysis. GitHub nevertheless moved CodeQL #45 to `main.rs:226`, the backup-topology stderr sink, on the new commit. The alert was not dismissed or called fixed; the next patch removes that sink's environment-derived output and must be verified on main. |
| 2026-09-25 | PR #113 and exact merged-main CodeQL closure | CodeQL #45 fixed; no open GitHub security alerts | PR #113 fixed the backup-topology stderr output without widening accepted topology. Local owning unit/integration tests, warnings-denied server Clippy and all 59 exact-head PR checks passed; merged as `d0dcd5d`. Separately dispatched full CI on that exact main commit passed, including GNU/all-features tests, mixed-feature soak, large-object and sustained replication, and Rust CodeQL. GitHub marked #45 fixed and reported zero open code-scanning, Dependabot and secret-scanning alerts on main. This records the automated gates, not a guarantee that every code path or deployment is vulnerability-free. |
