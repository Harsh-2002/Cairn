...
| 2026-09-25 | Trusted-proxy startup-error CodeQL #45 | Generic startup sink fix pending exact-head CI | `Config::trusted_proxy_allowlist` previously included a rejected `CAIRN_TRUSTED_PROXIES` value in its `ConfigError`; `main` printed that error to stderr. PR #111 made the proxy error fixed, but a fresh main CodeQL scan still reported the generic `Config::load()` error-print path. That sink is now fixed for all load/validation errors, including Figment parse errors. An owning real-binary regression checks the exact stderr for malformed proxy and bind-address values. Parsing and fail-closed trust decisions are unchanged. |
...
| 2026-09-25 | Recurrent WAL peak under mixed-feature soak | Candidate size-probe cadence correction; validation pending | The PR #112 initial strict soak again peaked at 709,756,552 bytes against the unchanged 536,870,912-byte ceiling. Its 33,073 operations had zero errors, byte-integrity and plateau checks passed. The 64-MiB size-triggered checkpoint was inspected only once per ten seconds, allowing hundreds of MiB of writes between probes. The candidate polls the existing sidecar once per second when the size trigger is enabled, retains the same checkpoint conditions and writer-thread busy behavior, and leaves the soak gate unchanged. A cadence regression and exact-head soak must pass before adopting it. |
