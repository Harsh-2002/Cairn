...
| 2026-09-25 | Trusted-proxy startup-error CodeQL #45 | Source path fixed; exact-head CI and main-branch alert closure pending | `Config::trusted_proxy_allowlist` previously included a rejected `CAIRN_TRUSTED_PROXIES` value in its `ConfigError`; `main` printed that error to stderr. A malformed value could therefore be retained by a service journal, including a secret accidentally pasted into that knob. The boundary now emits a fixed setting-specific diagnostic without the rejected value. An owning regression covers invalid host/CIDR entries and a sensitive-looking multiline value; no parsing or fail-closed trust decision changes. |
