...
| 2026-09-08 | Phase 3D restored-image ownership | Implemented; focused checks pass, integration pending | A canonical staged Writer establishes fresh storage ownership and clears copied coverage/claims before publication. The prepared inode, digest, schema and recovery tuple are checked around rename after checkpoint/close/synchronization; the source snapshot and manifest stay unchanged. Existing recorded-key identity/retirement checks run read-only before publication. Thirteen metadata and 25 server focused tests, owning Clippy and the encrypted live restore drill pass, including wrong-key refusal without changing existing target state. Schema remains v37; no baseline command or journal-only startup is enabled in this first 3D PR. |
