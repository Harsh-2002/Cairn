...
| 2026-09-08 | CRNB large-object index compatibility and memory | Limit fix PR #81 pending; bounded writer spool implemented locally, reader work remains | Writer and reader now share checked index-size arithmetic and known encoded lengths fail before staging/preallocation. The safe preflight regression fails against e38bc6c and passes after the fix; 47 blob unit and 38 integration tests pass. Nine-byte entries imply a maximum readable 488,671,805,440 logical bytes with 64-KiB encryption-only blocks, or 1,954,687,221,760 bytes with default 256-KiB compression blocks. These are code-derived boundaries, not huge-object test results. The 64-MiB reader cap remains; bounded index memory is subsequent work in `docs/storage-evolution-plan.md`. |
