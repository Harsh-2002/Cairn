...
const profileDir = await mkdtemp(join(tmpdir(), "cairn-console-performance-"));
const server = await createServer({
  server: { host: "127.0.0.1", port: 0 },
  plugins: [{
    name: "performance-fixture",
    configureServer(server) {
      server.middlewares.use("/__performance", async (_req, res) => {
...
if (browserErrors.length) throw new Error(browserErrors.join("\n"));
  ...
  chrome.kill("SIGTERM");
  await server.close();
  // Chromium finishes profile writes during exit; wait before removing the profile.
