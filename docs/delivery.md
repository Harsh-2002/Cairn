...
Releases are **calendar-versioned** (CalVer): the tag is `vYYYY.MM.DD`, exactly one release is ever active (the release workflow publishes today's tag, attaches the static `linux/amd64` and `linux/arm64` binaries with a `SHA256SUMS`, pushes the multi-arch container image as `:latest`, and retires every prior release and tag), and a release is a deliberate manual act gated on a green CI run for the exact commit being shipped. The crate's Cargo `version` (`0.1.0`) is an internal number and is **never** the user-facing version.
