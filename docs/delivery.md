...
Releases are **calendar-versioned** (CalVer): the tag is `vYYYY.MM.DD`, exactly one release is ever active (the release workflow publishes today's tag, attaches the static `linux/amd64` and `linux/arm64` binaries with a `SHA256SUMS` and the immutable container `IMAGE-DIGEST`, pushes the multi-arch container image as `:latest`, and retires every prior release and tag), and a release is a deliberate manual act gated on a green CI run for the exact commit being shipped. The crate's Cargo `version` (`0.1.0`) is an internal number and is **never** the user-facing version.

The release workflow is a staged trust boundary, not one broadly privileged build. Workflow-wide permissions are empty. Binary compilation, SBOM assembly, and OCI archive construction have read-only or no token authority and use full-commit action pins, exact tool versions with checksum verification, and digest-pinned base/BuildKit images. Those jobs emit fixed-name SHA-256 manifests and SLSA v1 predicates that bind each subject to the exact commit, once-computed CalVer, target, image name, invocation, and builder workflow; image provenance also records the two binary subjects included in the image. Separate jobs receive only the minimum authority for one operation: package write to copy the already-built OCI archive, OIDC/attestation authority to consume those predicates and sign subjects only after re-verifying their manifests and the registry digest, and contents write in the final job to create and retire releases only after verifying the complete signed bundle. No privileged job checks out, compiles, or executes build-artifact content. `tests/release_policy.py` enforces the immutable pins, exact permission map and dependency handoffs, verify-before-mutate ordering, and the rule that release/tag deletion occurs only in that final human-dispatched job; CI runs it before any package installer.
