...
The system exposes a liveness endpoint that succeeds whenever the process is serving and a readiness endpoint that succeeds only once migrations have applied, reconciliation has completed or been deliberately deferred, and the writer and read pool are responsive, so that an orchestrator does not route traffic to a process that is not yet ready. Read-pool responsiveness is a real, uncached `SELECT 1` on each configured metadata read pool, not a bucket listing, so its database and allocation cost is constant with respect to stored metadata. Health, readiness, and metrics remain observable when the application request budget is saturated through a separate fixed four-request infrastructure budget; they fail fast rather than queue when that budget is full, and metrics consumes at most two lanes so scrapes cannot occupy all readiness capacity. As an opt-in integrity facility, a background scrub (`CAIRN_SCRUB_INTERVAL_SECS`, **off by default** — with no interval configured no pass ever runs) re-reads stored blobs on the configured schedule and verifies their internal ingest SHA-256, falling back on legacy rows to stored full-object supplementary checksums or a single-part ETag, reporting a detected mismatch as `cairn_scrub_corruption_total` (with `cairn_scrub_objects_total` and `cairn_scrub_pass_seconds` for progress) rather than serving the corrupted byte. **Encrypted versions are verified like any other**: the pass unseals each version's data key and re-reads through it, so transparent at-rest, SSE-S3 and SSE-KMS objects are covered — the read authenticates every AES-GCM block and the recomputed plaintext digest is compared to the stored baseline.

Because a scrub that quietly verifies nothing is indistinguishable from a clean store, what a pass cannot verify is **counted and emitted, never dropped**: `cairn_scrub_skipped_total{reason}` breaks the residue down by `key_unavailable` (the version's key is not on the ring right now — a rotation window; skipped and retried next pass, deliberately **not** reported as corruption, since a false bit-rot alarm on a healthy store is its own incident), `io_error` (a transient filesystem failure — fd exhaustion, a full disk — skipped for the same reason and retried; a *missing* blob file, by contrast, is reported as `cairn_scrub_corruption_total{kind="missing_blob"}`), `composite_etag` (a legacy multipart row has neither an internal digest nor a full-object supplementary checksum; its composite ETag/checksum is a hash of hashes, so the object is read and authenticated but not content-hash verified), `no_blob`, `delete_marker`, and `metadata_unavailable`. A metadata listing that fails is counted as `cairn_scrub_enumeration_errors_total{stage}` (the pass still emits its log line rather than aborting silently, but covers less than the whole store). The invariant, which the pass log line (`scanned`, `skipped`, `corrupt`, `walked` and the per-reason fields) makes checkable, is that **`cairn_scrub_objects_total` + `cairn_scrub_skipped_total` account for every version the pass walked**; an operator alert should therefore look at both, since `scanned=0` with a non-zero `skipped` means coverage was lost, not that the store is clean. Note the scrub now re-reads and *decrypts* every version, so on an encrypted store a pass is CPU- and I/O-heavy in proportion to stored bytes (`CAIRN_SCRUB_BYTES_PER_SEC` optionally paces logical bytes between chunks; zero retains the unthrottled default, and only one blob-read permit is held at a time) — schedule the interval for quiet periods. Malformed stored baselines and mismatched logical lengths are corruption, not missing coverage. Shutdown cancels in-progress reads and pacing sleeps. The scrub complements the redundancy the operator provides at the storage layer by giving early warning of bit rot.
...
Management requests on either listener use route `api` and operation `Management`; browser-only routes rejected on the API listener remain `rejected`. Persistent downloads on either listener use route `share`, with the token redacted from request logs. They are not bucket mutations or a new high-cardinality metric family. Infrastructure endpoints retain their existing independent concurrency budget on the API listener.
