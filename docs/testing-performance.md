...
Each module is unit-tested against an in-memory double of its dependencies, so the tests are fast and deterministic. The pointed cases include the range-seek and prefix-upper-bound helpers against empty, maximal, and multibyte inputs, because they are the correctness of listing and pagination; the durable commit sequence, asserting through a seam that the destination directory is fsynced; multipart staging admission, proving session/part cardinality and byte quota are reserved before filesystem work and that replacement cleanup debt remains charged until deletion; path-exact multipart reconciliation inside a still-live session; the SigV4 canonicalisation and signing against the published test vectors; the totality of the error translator, enumerating every internal error and asserting a defined mapping; the authorization engine, table-driven across combinations of policy, ACL, ownership mode, and public-access-block settings with expected allow-or-deny outcomes; the ACL-to-action mapping and the canned-ACL expansion; and the chunked decoder against well-formed inputs. Protocol integration regressions additionally prove a missing exact UploadPart length and a quota rejection create no staging directory, while an actual-length mismatch leaves neither a file nor a reservation.

Dependency security is a per-commit gate rather than a periodic manual check. CI runs the pinned
`cargo-audit` scanner against `Cargo.lock`, runs `npm audit --omit=dev --audit-level=moderate` for
...
- **Partial covering index.** A partial index over current rows only (`WHERE is_latest = 1`) carrying the full `ListObjects` projection, including the internal lifecycle row identity as of migration v28, makes latest-only listing index-only — no per-row table fetch and no stepping over historical versions.
