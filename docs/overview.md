...
- **Object Lock (WORM)**: per-version retention (GOVERNANCE and COMPLIANCE modes) and legal hold, layered on versioning (see §16.5).
...
- **Lifecycle**: per-bucket rules for object expiration, noncurrent-version expiration, and aborting incomplete multipart uploads. A Transition rule is parsed but rejected at write time with `NotImplemented`; there is no cold-tier data movement and only the `STANDARD` storage class is exposed.
...
- **A storage-class hierarchy.** Cairn exposes only the `STANDARD` storage class and does not model the full matrix of S3 storage classes. Lifecycle transition to a remote cold tier is not supported: a Transition rule is rejected at write time with `NotImplemented`, and there is no data movement.
- **IAM as a standalone product.** Cairn has its own user and credential model and a bucket-policy and ACL engine sufficient for S3 authorization. It is not a general identity provider: IAM roles and federation are out of scope. The AWS-STS credential-minting surface (`AssumeRole` / `GetSessionToken`) is exposed for short-lived credentials (on by default; disable with `CAIRN_STS_ENABLED=false`), but the full AWS IAM surface is not. Identity-based policy is approximated by the user role model plus bucket policy; the boundaries are stated in Section 15.
