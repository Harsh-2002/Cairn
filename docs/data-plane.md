...
The API listener serves S3, native administrator JSON under `/api/v1`, STS, persistent shares and infrastructure endpoints. The optional console listener serves the embedded UI, the same management handlers, browser sessions/events and forced-download `/share/{token}` responses. Console cookies authenticate only console management requests. Native administration uses explicit credentials; disabling the console leaves API administration available. Object bytes can execute only on the separate API origin; console shares are always binary attachments. Immutable listener roles and an explicit route classifier are shared with the optimized GET path; neither unknown console paths nor native console-session paths can fall through to another route family.
