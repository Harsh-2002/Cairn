...
**Object integrity** adds a nullable internal hex SHA-256 to each object-version row. New ingest/assembly records it atomically with the blob reference; existing rows retain NULL and are never backfilled by a scrub. It is distinct from the client checksum JSON and excluded from serialized object responses.
...
Replication reserves that allowance before `open_raw_guarded`. The backend must retain its
`ReadBufferLease` through blocking work, any prepared-reader result, and streamed buffers,
including cancellation. Bounds belong to each implementation: the in-memory double emits a copied
range as one frame and must not borrow the filesystem decoder's assumptions.
