...
The metadata database's own durability is governed by its synchronous setting. Cairn defaults to the fully-synchronous setting, under which a committed transaction is durable against power loss, and exposes a relaxed setting as a documented throughput option under which the last few committed transactions may be lost on power loss without database corruption. The write-ahead log is checkpointed by a background task on an interval and when it exceeds a size threshold, using a truncating checkpoint so the log does not grow without bound under sustained writes. With the size trigger enabled, the task checks the WAL sidecar once per second (or at the shorter configured interval); with it disabled, it wakes only at the interval. The size threshold triggers a checkpoint but is not a hard WAL-size cap: writes between probes or a reader-pinned busy checkpoint can still cause a transient overshoot. The truncating checkpoint is dispatched onto the single writer thread so it never races a mutation, and because a stalled checkpoint would freeze every queued write it runs with its busy handler dropped: a checkpoint contended by an active WAL reader — the normal case, since the read pool holds long-lived snapshots — returns immediately as busy and is retried on the next probe rather than blocking the writer. Checkpoint runs, the count that returned busy, and the log size are observable as metrics so an operator can see whether long-lived readers are starving the checkpointer.
