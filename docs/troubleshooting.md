...
|---|---|---|---|
| `507 Insufficient Storage` on PUT / CreateMultipartUpload / UploadPart | Disk full, byte quota exhausted, or a multipart session/part ceiling reached | `df` on `CAIRN_DATA_DIR`; bucket/principal quota and `CAIRN_MULTIPART_MAX_*` settings; `cairn_*` disk metrics | Free space / grow the volume, raise the intended quota/ceiling, or abort abandoned uploads. Cairn rejects before staging when admission fails; cleanup debt stays charged until its bytes are actually removed. |
...
| Replica is behind / missing objects | Async replication lag, a down/stalled target, or the shared buffer budget limiting body concurrency | **`cairn_replication_unreplicated`**, oldest-pending age, per-target gauges; timeout warnings | Expected lag under load (eventually consistent). Check the target first. Then tune worker concurrency / batch size; if workers wait on large bodies, size `CAIRN_REPLICATION_BUFFER_BUDGET_BYTES` to available RAM rather than worker count. `CAIRN_REPLICATION_DELIVERY_TIMEOUT_SECS` cuts off request/head/body stalls and releases admission; a down target keeps queued work and resumes automatically. |
