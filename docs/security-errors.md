...
The browser-facing socket boundary is also a trust boundary. The console origin serves the embedded console, `/api/v1` and forced binary-attachment public downloads, and is the only listener that honours the administrator's httpOnly cookie. The API origin serves S3, explicit-credential native administration, STS, API shares, and infrastructure endpoints and never interprets that cookie as authentication. Because browser “site” identity does not include the port and may include sibling hostnames, `SameSite=Strict` alone is insufficient between those origins: the sign-in POST and every unsafe request that derives authentication from the cookie must carry the exact effective control `Origin`; missing or different origins fail before dispatch, while explicit `Authorization` clients remain unaffected. This also prevents login CSRF from replacing the victim's ambient console identity. Every embedded console document and asset response also refuses all framing with CSP `frame-ancestors 'none'` and `X-Frame-Options: DENY`, preventing potentially same-site active object content from clickjacking the cookie-authenticated console. Object bytes therefore cannot become active content in the privileged console origin; console transfers use exact, short-lived SigV4 URLs backed by durable scoped sessions. Data-origin object/share responses reject service-worker script fetches and constrain the maximum worker scope to a synthetic child of the exact request path, so one active object cannot install a persistent interceptor for a bucket directory or the shared `/share/` capability namespace. A reverse proxy must expose the two listeners as distinct origins, preserve the external control authority (`Host` or HTTP/2 `:authority`), and retain these security headers rather than recombine or strip the boundary; conflicting authority representations fail closed.

### 27.2 Transport security

Cairn can terminate TLS itself using a modern Rust TLS stack with current defaults and reloadable certificate material, so a deployment is secure on the wire without an external proxy (N-6), and it also runs behind a terminating proxy on a trusted interface. Direct TLS cannot be downgraded by forwarded metadata. On plaintext control connections, Cairn derives external HTTPS only from syntactically valid, non-conflicting `Forwarded` / single-valued `X-Forwarded-Proto` assertions delivered by an allow-listed immediate peer; every other case fails closed to HTTP for cookie attributes. For `aws:SourceIp`, an untrusted immediate peer's forwarding headers are ignored and the canonical socket address is used. An allow-listed peer must provide a valid `Forwarded` `for=` or `X-Forwarded-For` chain; Cairn walks it right-to-left only while predecessors are themselves allow-listed, and requires both families to resolve to the same canonical client when both are present. Missing, malformed, or conflicting provenance makes the condition key absent rather than treating the trusted proxy as the client, preserving CIDR-deny policies. The plaintext sendfile path falls back to the general request path when provenance is unavailable. The optional `fast-io` build can offload TLS record encryption to the kernel after a userspace handshake, while response bodies still pass through userspace. Zero-copy HTTPS reads are not implemented (Section 7.6). The control plane, including the web console and the CLI, is held to the same transport expectations as the data plane.
...
Ordinary PUT/Copy and multipart UploadPart/UploadPartCopy fail closed across request cancellation and commit-acknowledgement loss. Once staging returns durable bytes, a request-local guard owns the unique path together with either the intended immutable object-row identity or exact `(upload_id, part_number, attempt_id)`. Cancellation synchronously transfers that record to a retained worker, which resolves it through the serialized metadata writer after any original write: only an exact row/path miss authorizes deletion (and part-reservation release), an exact match preserves committed bytes, and a metadata error preserves the file for startup reconciliation. A delayed recovery cannot confuse a newer unversioned overwrite or same-number part retry with the original attempt. Every ordinary-object, part, and completion guard acquires a bounded slot before staging or claiming and transfers it through worker resolution, so nonblocking Drop recovery cannot become an unbounded in-memory queue. This avoids both quota-invisible live-process orphans and the worse inverse failure of unlinking bytes after their metadata transaction actually committed.

A global concurrency limit and per-request timeouts bound resource use under load, and the bounded blob pool and the streamed, backpressured transfers prevent a few large transfers from exhausting memory or threads. The asynchronous replication sink signs bounded logical-range streams in two passes, with one process-wide weighted buffer budget shared across every worker and target. Reader leases retain admission through actual blocking-I/O completion, including cancellation; CRNB geometry and physical lengths must match trusted metadata before block allocation. One non-resetting deadline covers admission, hashing and all multipart requests. Completion XML is capped at 16 MiB inside its 32 MiB allowance, and a destination can contribute at most 8 KiB to a retained response diagnostic (Section 20.4). The development authentication bypass is compiled out of release builds and additionally refuses non-loopback binds (F-17).

### 27.6 Residual risks

Several risks are acknowledged and accepted for the initial scope, recorded so they are known decisions rather than oversights. Object bytes are stored without whole-blob encryption unless server-side encryption is enabled for the object — SSE-S3, SSE-KMS, or transparent at-rest, each sealing a per-object data key under the master key (Section 27.8); an attacker with filesystem access can therefore read the content of any object not written under one of those modes, so the mitigations are enabling server-side encryption, which a bucket can additionally **mandate** via its encryption setting's `required` flag (Section 27.8, refusing any plaintext client PUT while transparently force-encrypting an inbound replica so replication is never broken), and full-disk or filesystem-level encryption provided by the operator; transparent object encryption is already available through `CAIRN_ENCRYPT_AT_REST`. SSE-KMS in this version is **label-only** — the key id is a validated label over the same master ring, not distinct key material — so it must not be relied on for cryptographic isolation between key ids (Section 27.8). A server-side-encrypted upload leaves nothing in plaintext on disk, including its in-flight multipart parts: each staged part is written as an encrypted block container under its own fresh per-part data key (itself sealed under the master key), and assembly decrypts the parts and re-encrypts the assembled object under its own data key — so a re-uploaded part gets a distinct key and an encrypted part is physically larger than its plaintext. The decision to encrypt parts is captured when the upload is initiated, so a bucket default added mid-upload applies only to the assembled object, not to that upload's already-staged parts; and if the master key sealing an in-flight part is retired before the upload completes, completion fails closed (the upload is retryable) rather than exposing plaintext. The authorization model supports per-user identity policies alongside the user role model, the bucket policy, and ACLs. It also supports scoped temporary credentials through AssumeRole/GetSessionToken, but does not implement cross-account principals or external identity federation (Section 15.1). Replicating to an endpoint exports data there, so the destination must be trusted and access-controlled; and because replication ships the **decrypted** body (the destination has a different master key and could not open the stored ciphertext), an object encrypted at the client's request is refused over a plaintext `http://` destination endpoint unless the operator opts in with `CAIRN_REPLICATION_ALLOW_PLAINTEXT_SSE_OVER_HTTP` (Section 20.4, Section 28) — an earlier implementation shipped such objects' raw ciphertext instead, which either failed the destination's digest check outright or produced a right-sized, unreadable replica. And single-node durability is the durability of the storage beneath Cairn, with cross-host durability provided only by asynchronous replication, which has lag (Section 8, Section 20).
...
A bucket may **mandate** encryption through the `encryption` aspect's `required` flag: a client PUT whose resolved plan is not an advertised SSE mode — plaintext or transparent at-rest — is refused, while an inbound replica is force-encrypted (SSE-S3) rather than refused, so enabling the policy never breaks replication into the bucket. Every seam fails closed: a missing or wrong master key or a tampered DEK envelope yields a decrypt error on open, never plaintext, zeros, or partial data. Blob framing comes only from the authoritative object/part metadata declaration, never from content sniffing. A declared plaintext file must have exactly the trusted logical length; this refuses the ordinary encrypted/uncompressed file if a caller loses its descriptor, and increments `cairn_blob_plaintext_length_mismatch_total`, while preserving arbitrary-byte S3 round-trips when a legitimate plaintext object's body is itself a complete CRNB file. The trusted descriptor also pins the encrypted-container generation: current object writes store `blob_format_version: 3`, and current multipart part envelopes carry the `crnb3:` prefix. Only metadata written by the legacy implementation — an absent object marker or unprefixed part envelope — selects v2. The mutable on-disk version byte cannot downgrade a current object into the unauthenticated v2 parser; an unknown explicit version or any metadata/container mismatch is corruption before object bytes are returned. Because v2 authenticates block bytes but not its index/trailer, its reader additionally binds the trailer algorithm and block size to the independently stored compression descriptor and enforces disjoint writer lengths for raw versus compressed entries after the GCM tag; an index flag or geometry mutation therefore fails before plaintext is returned. Part-level multipart encryption and the two fail-closed windows it entails (SSE intent pinned at initiate; master-key retirement mid-upload) are described in Section 27.6.

For legacy v2, the object row's `size_logical` or multipart part row's size also binds the
unauthenticated trailer/index logical total. This prevents a compressed-to-raw flag forgery from
becoming valid merely by shrinking both v2 length fields to the authenticated compressed payload.

---

Console share redemption authenticates only the token, independent of ambient cookies and unrelated authorization headers. It streams the existing scoped object-read response without an outbound HTTP request. Object metadata and query overrides cannot change the forced `application/octet-stream` and attachment delivery. Responses set `nosniff`, no-store, no-referrer, frame denial and sandbox CSP, strip cookies, reject service-worker script requests, and redact the capability from logs/errors. Unknown tokens, revoked/expired shares and missing versions retain their existing read failure semantics. Browser uploads and inline previews still require access to the API origin.
